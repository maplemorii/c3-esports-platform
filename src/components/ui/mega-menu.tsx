            ...
            <AnimatePresence>
              {openMenu === navItem.label && navItem.subMenus && (
                <div className="absolute left-0 top-full w-auto pt-2 z-10">
                  <motion.div
                    className="w-max border border-white/10 p-4"
                    style={{
                      borderRadius: 16,
                    ...
                    <div className="flex w-fit shrink-0 space-x-9 overflow-hidden">
                      {navItem.subMenus.map((sub) => (
                        <motion.div layout className="w-full" key={sub.title}>
                          <h3 className="mb-4 text-sm font-medium capitalize text-white/40 font-sans tracking-widest text-[10px] uppercase">
                            {sub.title}
                          </h3>
                            ...
                            {sub.items.map((item) => {
                              ...
                              return (
                                <li key={item.label}>
                                  ...
                                  >
                                    <div className="flex size-9 shrink-0 items-center justify-center rounded-md border border-white/15 text-white/50 transition-all duration-300 group-hover:border-[rgba(196,28,53,0.45)] group-hover:bg-[rgba(196,28,53,0.10)] group-hover:text-white">
                                      <Icon className="h-4 w-4 flex-none" />
                                    </div>
                                    <div className="w-max leading-5">
                                      <p className="shrink-0 text-sm font-medium text-white/80 group-hover:text-white transition-colors duration-200">
                                        {item.label}
                                      </p>
                                      <p className="shrink-0 text-xs text-white/35 transition-colors duration-200 group-hover:text-white/60">
                                        {item.description}
                                      </p>
                                    </div>
                                  </Link>
